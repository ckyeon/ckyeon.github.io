...
> 이 게시물은 Chirpy Tutorial의 [Writing a New Post](https://chirpy.cotes.page/posts/write-a-new-post/)를 번역한 글입니다.
...
게시물의 업로드 날짜를 정확하게 기록하기 위해 `_config.yml` 파일에서 `timezone`을 설정하고,

```yaml
timezone: Asia/Seoul
```

Front Matter 블록의 `date`에 UTC/GMT 시간을 지정해야 한다.
...
기본적으로, **T**able **o**f **C**ontents (ToC)는 게시물의 오른쪽 패널에 표시된다.

전체 게시물의 ToC를 끄려면 `_config.yml` 파일에서 `toc`를 `false`로 설정하면 된다.

만약 특정 게시물에 대한 ToC를 끄려면 **`Front Matter`**에 다음을 추가하면 된다.
...
ToC와 비슷하게 [Disqus](https://disqus.com/) 댓글은 기본적으로 각 게시물에 로드된다. 

댓글의 전역 스위치는 `_config.yml` 파일의 `comments` 변수로 정의된다. 

만약 특정 게시물에 대한 댓글을 닫으려면 **`Front Matter`**에 다음을 추가하면 된다.
...
웹사이트의 성능상 이유로, 수학 기능은 기본적으로 로드되지 않는다. 

게시물에서 **`Mathematics`** 기능을 활성화 하려면 게시물의 **`Front Matter`**에 다음을 추가하면 된다.
...
[**Mermaid**](https://github.com/mermaid-js/mermaid)는 훌륭한 다이어그램 생성 도구다. 

게시물에서 **`Mermaid`** 기능을 활성화하려면 YAML 블록에 다음을 추가하면 된다.
...
`Mermaid`는 다른 마크 다운 언어처럼 사용할 수 있다. 

그래프 코드를 ```` ```mermaid ```` 및 ```` ``` ````로 묶으면 된다.
...
```markdown
![img-width](/assets/img/sample/mockup.png){: width="300"}
```

![img-width](/assets/img/profile/cat.jpg){: width="300"}
...
- **Normal Position**
  ...
  ```markdown
  ![img-position-normal](/assets/img/sample/mockup.png){: .normal}
  ```

- **Float to the Left**

  ```markdown
  ![img-position-left](/assets/img/sample/mockup.png){: .left}
  ```

- **Float to the Right**

  ```markdown
  ![img-position-right](/assets/img/sample/mockup.png){: .right}
  ```
...
코드 블럭에 ```` ```language ````를 사용하면 줄 번호와 구문 강조 표시가 있는 코드 블록을 만들 수 있다.
