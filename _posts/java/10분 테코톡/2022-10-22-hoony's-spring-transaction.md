...
@Repository
public class RobotRepositoryImpl {
    public void move(Connection conn, Long id, Position position) throws SQLException {
        Connection conn = null;
...
하지만, 고수준 명세인 `RobotRepository`가 저수준 모듈 `RobotRepositoryImpl`에 의존하게 되어버린다.

즉, 추상화가 세부 명세에 의존하게 되어

**DIP가 깨지게 되는 것이다.**

단순히 JDBC를 JPA로 변경해보자.  
변경된 기술에 따라 저수준 모듈을 바꾸었을 뿐인데 고수준 명세인 `RobotRepository`와 `RobotService`까지 변경해야 하는 일이 발생한다.
```java
public interface RobotRepository {
...
## 스프링 트랜잭션 기술
스프링은 문제점을 개선할 수 있도록 3가지 스프링 트랜잭션 관리 기법을 선보인다.
1. 트랜잭션 동기화
2. 트랜잭션 추상화
3. 선언적 트랜잭션
